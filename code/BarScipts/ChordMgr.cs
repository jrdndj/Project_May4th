...
public class ChordMgr : MonoBehaviour
{
    ...
    static List<int> G7sa = new List<int>() { 28, 33, 36 }; //removed 31 here cos of overlap
    ...
    //you need a mapping function for each chord from CollectionMgr and to here

    public void ChordMapper()
    {
        List<List<int>> ChordListToSend = new List<List<int>>();
        List<List<int>> LickListToSend = new List<List<int>>();
        List<int> LengthListToSend = new List<int>();

        //so we need an instance of CollectionMgr here and RollScript
        //get the string list and get the list of lengths
        CollectionManager.GetComponent<CollectionMgr>().SendToChordMgr();

        //now the mapping happens 

        //have a return later that we will send to RollScript
    }


    //==== these variables are decommissioned.
    //==== we get the sequences from CollectionMgr 
    ...
    //List<List<int>> ChordList = new List<List<int>>() { Dm7, G43Chord, Cmaj7Chord, A7Chord, Dm7, G43Chord, Cmaj7Chord, Cmaj7Chord };
    //List<List<int>> LickList = new List<List<int>>() { Dm7CT, G43ChordTone, Cmaj7ChordTone, A7ChordTone, Dm7CT, G43ChordTone, Cmaj7ChordTone, Cmaj7ChordTone };
    //List<List<int>> HalfStepList = new List<List<int>>() { Dm7HS, G43HalfStep, Cmaj7HalfStep, A7HalfStep, Dm7HS, G43HalfStep, Cmaj7HalfStep, Cmaj7HalfStep };
    //List<List<int>> StepAboveList = new List<List<int>>() { Dm7SA, G43Above, CMaj7Above, A7Above, Dm7SA, G43Above, CMaj7Above, Cmaj7HalfStep };
