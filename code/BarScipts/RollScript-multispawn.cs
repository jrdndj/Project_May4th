...
public class RollScript : MonoBehaviour
{
    ...
    float barSpeed = (float)0.67; //from 0.05 0.65 was ok //0.15 is still too fast
    //based on the fixed update rate
    ...
    //==== SET 05 Chords
    List<String> ChordTimings = new List<String>() { "00:02", "00:03", "00:04", "00:05",
                                                    "00:20", "00:24", "00:28", "00:32" };
    List<string> ChordNames = new List<string>() { "Dmin7", "G7", "Cmaj7", "A7",
                                                    "Dmin7", "G7", "Cmaj7", "Cmaj7" };
    List<List<int>> ChordList = new List<List<int>>() { Dmin7Chord, G43Chord, Cmaj7Chord, A7Chord,
                                                        Dmin7Chord, G43Chord, Cmaj7Chord, Cmaj7Chord };
    List<List<int>> LickList = new List<List<int>>() { Dmin7ChordTone, G43ChordTone, Cmaj7ChordTone, A7ChordTone,
                                                        Dmin7ChordTone, G43ChordTone, Cmaj7ChordTone, Cmaj7ChordTone };
    List<List<int>> HalfStepList = new List<List<int>>() { Dmin7HalfStep, G43HalfStep, Cmaj7HalfStep, A7HalfStep,
                                                            Dmin7HalfStep, G43HalfStep, Cmaj7HalfStep, Cmaj7HalfStep };
    List<List<int>> StepAboveList = new List<List<int>>() { Dmin7Above, G43Above, CMaj7Above, A7Above,
                                                            Dmin7Above, G43Above, CMaj7Above, Cmaj7HalfStep };
    ...
    void Update()
    {
        ...
        //change this to a time 
        //if (spawnNew)
        //if (String.Compare(time.ToString(@"mm\:ss\:fff"), ChordTimings[ctr].ToString()) == 0)
        if (String.Compare(time.ToString(@"mm\:ss"), ChordTimings[ctr]) == 0)
        {
            //clear lines when we spawn new
            //clearMapLines();
